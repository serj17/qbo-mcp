...
- **Core libraries:** `@modelcontextprotocol/sdk` (MCP server framework), `intuit-oauth` (official Intuit OAuth library — used for the auth ceremony and access-token refresh only), `pino` (structured logging), `env-paths` (OS-appropriate config/cache/log dirs), `zod` (tool input validation), `vitest` (testing), `dotenv` (loads `.env` for app credentials), `open` (browser launch for OAuth callback flow).
- **QBO HTTP layer:** hand-rolled `fetch` against the QBO REST API at `https://{sandbox-,}quickbooks.api.intuit.com/v3/company/{realm}/...` with `Authorization: Bearer <access_token>` headers. Reversed an earlier "use `node-quickbooks`" decision after wiring it up — the library has no public raw-QBQL passthrough, its callback API needs promisification per method, and its 10-deep transitive-dep tree (incl. `bluebird`, `underscore`, `oauth-1.0a`, `eval()` calls) doesn't insulate from anything that's actually changing in the QBO API. ~50 extra lines in `qbo-client` for full control of URL building, error mapping, and the silent refresh-and-retry-once policy.
