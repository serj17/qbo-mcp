...
- [x] Both tools registered via `tool-registry.defineTool({ ... })`
- [x] Zod schemas document accepted date formats and the meaning of `accounting_method` and `summarize_by`
- [x] Default `accounting_method` is `Accrual` (matches QBO UI default)
- [x] Tools return the QBO Report JSON unmodified
- [x] One sandbox integration test per tool, asserting a non-empty `Rows` block in the response
