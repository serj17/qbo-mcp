...
/**
 * Integration test against the QBO sandbox. Opt-in via env var so the default
 * `npm test` run remains offline:
 *
 *   QBO_MCP_INTEGRATION_TESTS=1 npm test
 *
 * Requires a completed `npx qbo-mcp auth --env=sandbox` so tokens.json exists.
 */
...
describe.skipIf(skip)("list_invoices integration (sandbox)", () => {
  it("returns up to 5 invoices with the documented page_info shape", async () => {
    const config = getConfig();
    ...
    const result = await handleListInvoices({ limit: 5 }, qbo);
    ...
    // Spot-check that items look like Invoice entities (Id is the universal QBO key).
  ...
  it("count_only returns total without items", async () => {
    ...
    const result = await handleListInvoices({ count_only: true }, qbo);
