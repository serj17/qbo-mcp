...
describe.skipIf(skip)("list_customers integration (sandbox)", () => {
  it("returns up to 5 customers with the documented page_info shape", async () => {
    const config = getConfig();
    ...
    const result = await handleListCustomers({ limit: 5 }, qbo);
  ...
  it("count_only returns total without items", async () => {
    ...
    const result = await handleListCustomers({ count_only: true }, qbo);
