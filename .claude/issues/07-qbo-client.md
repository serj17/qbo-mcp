...
- [x] Single retry on 401 with refresh; second 401 returns `AUTH_REFRESH_FAILED`
- [x] Rotated refresh token is persisted before the retry (so a crash between refresh and retry doesn't lose the new refresh token)
- [x] HTTP 429 maps to `RATE_LIMITED` with `retry_after_seconds` parsed from the `Retry-After` header
- [x] HTTP 404 / QBO `ObjectNotFound` fault maps to `NOT_FOUND`
- [x] QBO `ValidationFault` / `BusinessValidationFault` maps to `INVALID_INPUT` with the QBO message preserved
- [x] HTTP 5xx maps to `QBO_SERVER_ERROR` with `retryable: true`
- [x] Network/socket errors map to `NETWORK_ERROR` with `retryable: true`
- [x] Bad QBQL syntax maps to `INVALID_QUERY` with the QBO error verbatim
- [x] Each call logs a request + response line at `info`, errors at `error` with full QBO fault payload
- [x] Unit tests on the error mapper alone, using fixture QBO error responses (no network)
