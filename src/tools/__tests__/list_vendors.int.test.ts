...
describe.skipIf(skip)("list_vendors integration (sandbox)", () => {
  it("returns up to 5 vendors with the documented page_info shape", async () => {
    const config = getConfig();
    ...
    const result = await handleListVendors({ limit: 5 }, qbo);
  ...
  it("count_only returns total without items", async () => {
    ...
    const result = await handleListVendors({ count_only: true }, qbo);
