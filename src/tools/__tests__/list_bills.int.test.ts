...
describe.skipIf(skip)("list_bills integration (sandbox)", () => {
  it("returns up to 5 bills with the documented page_info shape", async () => {
    const config = getConfig();
    ...
    const result = await handleListBills({ limit: 5 }, qbo);
  ...
  it("count_only returns total without items", async () => {
    ...
    const result = await handleListBills({ count_only: true }, qbo);
